...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submit_register'])) {
    ...
    $emailExist = db_fetch_row("SELECT * FROM `users` WHERE `usr_email`='$usr_email'");
    $phoneExist = db_fetch_row("SELECT * FROM `users` WHERE `usr_phone`='$usr_phone'");

    if ((isset($emailExist) && !empty($emailExist)) || (isset($phoneExist) && !empty($phoneExist))) {
        // Tồn tại
        $error_messages = "Email hoặc số điện thoại đã tồn tại";
    } else {
        // Tạo người dùng
        $data = [
            ...
            "usr_phone" => $usr_phone,
            "usr_email" => $usr_email,
            "usr_password" => password_hash($usr_password, PASSWORD_BCRYPT), // Mã hóa password
        ];

        db_insert("users", $data);

        // Sau khi tạo thành công chuyển hướng đến trang đăng nhập
    ...
    <div id="wrapper">
        ...
        <main class="main-site main-childs">
            <div class="user-wrapper">
                ...
                <form accept-charset="UTF-8" id="formAcount" action="" class="validate" method="post">
                    <!-- <input type="hidden" value="17181b2d420931fb860782832ab2dd3b-6b868767e8bd54322aa5050f077f50fb" name="csrf" id="csrf" /> -->
                    <div class="form-group">
                        <input type="text" class="form-control validate[required]" id="fullName" name="usr_fullName" value="<?= $usr_fullName ?>" placeholder="Họ tên" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control validate[required, custom[phone]]" id="mobile" name="usr_phone" value="<?= $usr_phone ?>" placeholder="Điện thoại" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <input type="email" class="form-control validate[required, custom[email]]" id="email" name="usr_email" value="<?= $usr_email ?>" placeholder="Email" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <input type="password" class="form-control validate[required]" id="password" name="usr_password" value="<?= $usr_password ?>" placeholder="Mật khẩu của bạn" autocomplete="off">
                    </div>
                    ...
                    <button type="submit" id="btnsingup" name="submit_register" class="btn btn-organ w__100">Đăng ký</button>
                </form>
            </div>
        </main>
        <style>
                    ...
                    <div class="row">
                        <div class=" copy">
                            ...
                            <div class="col-xs-12 col-sm-8">
                                ...
                                <div class="text-left" style=" display: block" data-runtime="Memory:4.72 mb - Time:0.14s">
